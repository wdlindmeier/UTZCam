...
- (UIImage *)cropImageToRect:(CGRect)cropRect 
{
	...
	*/
    ...
    // NEW (with image scaling)
    CGRect scaledCropRect = CGRectMake(cropRect.origin.x * self.scale, 
                                       cropRect.origin.y * self.scale, 
                                       cropRect.size.width * self.scale, 
                                       cropRect.size.height * self.scale);
    CGImageRef imageRef = CGImageCreateWithImageInRect([self CGImage], scaledCropRect);    
    UIImage *ret = [UIImage imageWithCGImage:imageRef scale:self.scale orientation:self.imageOrientation];
...
{
	CGSize newSize = CGSizeMake(ceil(image.size.width * scaleRatio * image.scale),
                                ceil(image.size.height * scaleRatio * image.scale));
    ...
    /*
    // NEW (with image scaling)
    CGRect scaledCropRect = CGRectMake(0, 0, newSize.width * image.scale, newSize.height * image.scale);
